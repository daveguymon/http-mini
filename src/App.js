...
class App extends Component {
  ...
  recruitTroop(event) {
      ...
      if(this.state.newRecruit) {
          addTroop(this.state.newRecruit).then(() => { this.callTroops() })
  ...
  render() {
      ...
      const enemies = this.state.enemies.map((enemy, i) => (
        <ul className="army">
          ...
          <div onClick={() => this.slayLeader(enemy.shortname, enemy.id)} className="leader">
          {enemy.leader}
          </div>
          
          <ul className="minions">
          {enemy.minions.map((minion, i) => (<li onClick={ () => {this.transformMinion(enemy.shortname, minion.id)}} className="minion" key={i}>{minion.type}</li>))}
